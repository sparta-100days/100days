...
class OAuth2Service(
    ...
    fun login(authorizationCode: String): String {
        return oAuth2Client.getAccessToken(authorizationCode)
            .let { oAuth2Client.retrieveUserInfo(it) }
            .let { userService.registerIfAbsent(it) }
            .let { jwtPlugin.accessToken(it.id!!, it.email, it.role) }

        /*
        // 소셜로그인측 : 인가코드 > 액세스 토큰 발급
        val accessToken = oAuth2Client.getAccessToken(authorizationCode)
        // 소셜로그인측 : 액세스 토큰으로 사용자정보 조회
        val userInfo = oAuth2Client.retrieveUserInfo(accessToken)
